...
{
	Linfo << "This is run photon task set...";
	PyUtil::Args args(s);
	...
	GenUtil::SubjetTask sj_ca_sjR10("photon_subjets_ca_sjR10", (args.asString() + " --sjA=1 --sjR=0.10").c_str());
	sj_ca_sjR10.AddInputTask(r);
	r->AddTask(&sj_ca_sjR10);

	// Photons::PhotonTask photon_general("photon_general", (args.asString() + " --jetR=0.4 --eta=10.").c_str());
	Photons::PhotonTask photon_general("photon_general", args.asString().c_str());
	photon_general.AddInputTask(r);
	r->AddTask(&photon_general);
	...
	delete r;
	Linfo << "photon task set done." << endl;
	return 0;
}
